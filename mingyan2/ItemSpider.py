...
#         ana = response.css('div.quote')[0]  # ana 名言集
#
#         text = ana.css('.text::text').extract_first()  # 提取名言
#         author = ana.css('.author::text').extract_first()  # 提取作者
#         tags = ana.css('.tags.tag::text').extract()  # 提取签名
#         tags = ','.join(tags)  # 数组转换为字符串
#
#         filename = '%s - 语录.txt' % author  # 爬去的内容存到文件，文件名为：作者-语录.txt
